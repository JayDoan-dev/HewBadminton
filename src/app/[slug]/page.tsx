...
const SinglePage = async ({ params }: { params: { slug: string } }) => {
  ...
  return (
    <div className="px-4 md:px-8 lg:px-16 xl:px-32 2xl:px-64">
      {/* Responsive Breadcrumb on top, always visible */}
      <div className="w-full pt-2 pb-1 md:pt-4 md:pb-2">
        <Breadcrumb
          items={[
            { name: "Home", href: "/" },
            { name: "Shop", href: "/list?cat=all-products" },
            { name: product.name || "Product", href: `/[slug]?slug=${product.slug}` },
          ]}
      ...
      <div className="relative flex flex-col lg:flex-row gap-16">
        {/* IMG */}
        <div className="w-full mt-4 lg:w-1/2 lg:sticky top-20 h-max">
